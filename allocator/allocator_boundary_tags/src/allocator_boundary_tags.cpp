...
{
    *reinterpret_cast<fit_mode *>(reinterpret_cast<u_char *>(_trusted_memory) + sizeof(allocator *) + sizeof(logger *) +
				  sizeof(size_t));
}
...
std::vector<allocator_test_utils::block_info> allocator_boundary_tags::get_blocks_info() const noexcept
{
    ...
    {
	size_t block_size =
		reinterpret_cast<u_char *>(target_block) - reinterpret_cast<u_char *>(get_memory_begining());
